...
app.post("/customer", async(req, resp) =>
{
  console.log(req.body.param);
    try
    {
      let result = await sql.execute("insert into customers set ?", [req.body.param]);
      console.log(result);
      resp.json(result); // 웹페이지에 출력
    }
    catch(err)
...
app.delete("/customer/:id", async(req,resp) =>
{
  ...
  try
  {
    let remove = await sql.execute("delete from customers where id=?", [req.params.id]);
    console.log(remove);
...
app.put("/customer/:id", async(req,resp) =>
{
  ...
  try
  {
    let modify = await sql.execute("update customers set ? where id=?", [req.body.param, req.params.id]);
    console.log(modify);
